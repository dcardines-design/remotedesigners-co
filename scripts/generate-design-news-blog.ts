/**
 * Generate a design-news blog post with Ghibli watercolor style image
 */

import dotenv from 'dotenv'
dotenv.config({ path: '.env.local' })

async function generateDesignNewsBlog() {
  ...
  console.log('=== Generating Design News Blog ===\n')

  // 1. Generate unique topic for design-news category
  console.log('1. Generating unique topic...')
  const topic = await generateUniqueTopic('design-news')
  ...
  const image = await generateBlogImage('design-news', content.featured_image_alt, 5)
  if (image) {
...
generateDesignNewsBlog().catch(console.error)
