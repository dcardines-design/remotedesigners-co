...
async function checkIndeedQuota(): Promise<{ available: boolean; message: string }> {
  ...
  try {
    // Make minimal test call (1 result) to check quota
    ...
    if (res.status === 429) {
      return { available: false, message: 'Rate limited - too many requests' }
    }

    if (res.status === 403) {
      return { available: false, message: 'Quota exceeded or subscription issue' }
    }

    if (!res.ok) {
      return { available: false, message: `API error: ${res.status}` }
    }
